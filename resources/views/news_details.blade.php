...
<meta name="image" property="og:image" content="@foreach($news->images as $image) {!! asset("storage/images/news/$image->filename") !!} @break @endforeach ">
<meta property="og:image" content="@foreach($news->images as $image) {!! asset("storage/images/news/$image->filename") !!} @break @endforeach ">
<meta name="description" property="og:description" content="{{ $news->description }}">
<meta property="og:type" content="article" />

<meta name="keywords" property="og:keywords" content="@foreach(json_decode($news->keywords, true) as $key => $value) {{ $value['value'] }}, @endforeach"/>
<meta name="news_keywords" property="og:news_keywords" content="@foreach(json_decode($news->keywords, true) as $key => $value) {{ $value['value'] }}, @endforeach" />

@endsection

@section('css')
    @if(App::getLocale() == 'en')
        <link rel="stylesheet" href="{{ asset('website/css/en/news_details.ltr.css') }}">
...
@section('content')
  <div class="containerr">
    <section class="hedsid">
      <h6><a href="{{ route('homePage') }}">{{ __('master.home') }} / </a><a href="{{ route('news') }}">{{ __('nav.news and announcements') }}  / </a></h6>
      <h6> {{ __('master.news details') }} </h6>
    </section>
        ...
        <div class="right_newsdeta">
          ...
          <div class="icons_ns">
            <div class="icon_n">
              ...
              <p>{{ $news->updated_at->toDateString() }}</p>
            </div>
            <div class="icon_n">
              ...
              <p>{{ $news->updated_at->toTimeString() }}</p>
            </div>
            <div class="icon_n">
              ...
              <p>{{ __('master.palestine') }} - {{ __('master.gaza') }}</p>
            </div>
          </div>
              ...
              <div class="swiper-wrapper">
                  ...
                  @foreach($news->images as $image)
                      <div class="swiper-slide imgnewsd">
                          <img src="{{ asset("storage/images/news/$image->filename") }}" alt="">
                      </div>
                  @endforeach
            ...
            <div class="sotial_news">
              ...
              ?>
                <div class="icon_sot">
                    ...
                    <a href="https://t.me/share/url?url={{ $url }}" class="socialIconD">
                        <i class="bx bxl-telegram" style='color:#ffffff'></i>
                    </a>
                    <a href="https://twitter.com/intent/tweet?url={{ $url }}/" class="socialIconD">
                        <i class="bx bxl-twitter" style='color:#ffffff'></i>
                    </a>
                    <a href="whatsapp://send?text={{ $url }}" class="socialIconD">
                  ...
                  <div class="lastnews">
                      <div class="imglnews">
                          @foreach($new->images as $image)
                            <img width="150" height="90" src="{{ asset("storage/images/news/$image->filename") }}" alt="">
                            @break
                          @endforeach
                      </div>
                      <div class="textlnews">
                          ...
                          <div class="nononews">
                              ...
                              <p>{{ $new->updated_at->toTimeString() }}</p>
                          </div>
                      </div>
                  </div>
              @endforeach
    ...
    <div class="keyword">
      <h3>{{ __('master.keywords') }}</h3>
      <hr>
      <div class="alltagkey">

       @foreach(json_decode($news->keywords, true) as $key => $value)
          <div class="tagkey">
              <p>{{ $value['value'] }}</p>
          </div>
       @endforeach
